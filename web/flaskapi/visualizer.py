...
def send_subjects (subject_name):
    f = open ('scraped_course_data.json')
    course_information = json.load(f)
    importedSubjects = json.load (f)
    subjectList = []
    for subject in importedSubjects['courses']:
        temp = subject['code'].split('*')[0]
        subjectList.append(temp)
    finalSubjectList = set(subjectList) #pass this to function call
    f.close()

    return create_graph('', subject_name, False, None, False, 'temp/', True, '', finalSubjectList, True, course_information, None)
